...
import {
  handler as resetRatings,
  setDbPool as setResetPool,
  resetDbPool as resetResetPool,
  setJwtVerifier as setResetVerifier,
  resetJwtVerifier as resetResetVerifier,
  setBroadcaster as setResetBroadcaster,
  resetBroadcaster as resetResetBroadcaster,
} from "../../supabase/functions/admin-reset-ratings/index.ts";
...
describe("admin-reset-ratings", () => {
  const okStubs = (overrides: QueryStub[] = []): QueryStub[] => [
    ...overrides,
    ["FROM matches WHERE status NOT IN", []],
    ["SELECT initial_rating FROM system_settings", [{ initial_rating: 1500 }]],
    ["UPDATE teams SET rating", [{ id: "team-1" }, { id: "team-2" }]],
  ];

  it("falls back to the configured initial rating", async () => {
    // TC-ADMIN-038 / TC-ADMIN-040 / TC-ADMIN-046
    setResetVerifier(adminVerifier);
    const sent: { channel: string; event: string }[] = [];
    setResetBroadcaster((channel, event) => {
      sent.push({ channel, event });
      return Promise.resolve();
    });
    const db = createMockDb(okStubs());
    setResetPool(db.pool as never);

    try {
      const res = await post(resetRatings, {});
      assertEquals(res.status, 200);
      assertEquals((await res.json()).data, { affectedTeams: 2, initialRating: 1500 });
      assertEquals(sent, [{ channel: "ranking", event: "RANKING_UPDATED" }]);
    } finally {
      resetResetVerifier();
      resetResetPool();
      resetResetBroadcaster();
    }
  });

  it("uses the provided initial rating", async () => {
    // TC-ADMIN-039
    setResetVerifier(adminVerifier);
    setResetBroadcaster(() => Promise.resolve());
    const db = createMockDb(okStubs());
    setResetPool(db.pool as never);

    try {
      const res = await post(resetRatings, { initialRating: 1200 });
      assertEquals((await res.json()).data.initialRating, 1200);
      assertEquals(db.find("UPDATE teams SET rating")!.params, [1200]);
    } finally {
      resetResetVerifier();
      resetResetPool();
      resetResetBroadcaster();
  ...
  it("does not write rating history for a reset", async () => {
    // TC-ADMIN-041 / TC-ADMIN-042
    // ★rating_history.match_id は NOT NULL の外部キーであり、試合に紐づかない履歴は作れない（ADR-017）。
    setResetVerifier(adminVerifier);
    setResetBroadcaster(() => Promise.resolve());
    const db = createMockDb(okStubs());
    setResetPool(db.pool as never);
    ...
    try {
      await post(resetRatings, {});
      assertEquals(db.find("INSERT INTO rating_history"), undefined);
      assertEquals(db.find("DELETE FROM rating_history"), undefined);
    } finally {
      resetResetVerifier();
      resetResetPool();
      resetResetBroadcaster();
  ...
  it("refuses to reset while a match is in progress", async () => {
    // TC-ADMIN-043 / TC-ADMIN-044
    setResetVerifier(adminVerifier);
    const db = createMockDb(okStubs([["FROM matches WHERE status NOT IN", [{ id: "match-1" }]]]));
    setResetPool(db.pool as never);

    try {
      const res = await post(resetRatings, {});
      assertEquals(res.status, 409);
      assertEquals((await res.json()).error.code, "RATING-003");
  ...
  it("rejects an initial rating below the bound", async () => {
    // TC-ADMIN-045
    setResetVerifier(adminVerifier);
    const db = createMockDb(okStubs());
    setResetPool(db.pool as never);
    ...
    try {
      const res = await post(resetRatings, { initialRating: 99 });
      assertEquals(res.status, 400);
      assertEquals((await res.json()).error.code, "RATING-002");
    } finally {
      resetResetVerifier();
      resetResetPool();
    }
  ...
  it("records the rating reset", async () => {
    // TC-ADMIN-050
    setResetVerifier(adminVerifier);
    setResetBroadcaster(() => Promise.resolve());
    const db = createMockDb(okStubs());
    ...
    try {
      await post(resetRatings, {});
      const audit = db.find("INSERT INTO audit_logs")!;
      assertStringIncludes(audit.sql, "'RATING_RESET'");
      assertEquals(audit.params[0], "admin-1");
    } finally {
      resetResetVerifier();
      resetResetPool();
