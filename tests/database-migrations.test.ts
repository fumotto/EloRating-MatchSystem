...
Deno.test("Database Migrations - Tables and Views are correctly defined", async () => {
  ...
  for (const table of expectedTables) {
    assertStringIncludes(allContent, `CREATE TABLE ${table} (`, `Table '${table}' should be defined`);
  }
  ...
  for (const view of expectedViews) {
    assertStringIncludes(allContent, `CREATE VIEW ${view} AS`, `View '${view}' should be defined`);
  }

  // Check that migration files are numbered sequentially
  ...
  for (let i = 0; i < sortedFiles.length; i++) {
    ...
    assertStringIncludes(sortedFiles[i], expectedFileName, `Migration file ${i+1} should be named correctly`);
  }
  ...
  for (const func of forbiddenFunctions) {
    assertEquals(allContent.toLowerCase().includes(func), false, `Function '${func}' should not be defined`);
  }
});
