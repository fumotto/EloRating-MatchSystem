...
* **サブアカウント対策は前処理で無効化する**（ADR-036 ⑤）。`tests/e2e/global-setup.ts` が
  `rematchCooldownHours` と `rankingMinOpponents` を 0 にする。**ここが本番と検証環境の唯一の差である。**
  無効化しないと、1試合しかしないシナリオのチームがランキングに現れず 3.3 と 3.7 が落ちる。
...
* 設定の変更は `admin-update-system-settings` を通す。`service_role` で `system_settings` を
  直接書き換えない。0013_rls.sql はどのクライアントロールにも書き込みを与えていない（ADR-016）。
* **Seed の既定値を検証環境向けに変えない**（ADR-036 ⑤）。既定値を環境ごとに分けると、
  CI で通ったものが本番では違う挙動になる。差は前処理で付ける。
