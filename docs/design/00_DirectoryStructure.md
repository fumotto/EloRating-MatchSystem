...
**本ディレクトリの内容はビルド時に `dist/` へそのまま複製され、公開される。**
配信する必要のないファイルを置いてはならない。

## 8.1 assets/

**配信しない素材**を配置する。ビルド生成物の元データが対象である。

例

* favicon の元画像（`assets/favicon-source.png`）

`public/` と分ける理由は容量ではなく責務である。`public/` は「配信するもの」、
`assets/` は「配信物を作るための元データ」であり、後者は利用者へ届ける必要がない。

置き場所に迷ったら「ブラウザがこのURLを直接取得することがあるか」で判断する。
無ければ `assets/` である。
