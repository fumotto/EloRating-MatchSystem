...
**CI は動かさない。** 検証は PR で済んでおり、merge のたびに同じ内容をもう一度走らせない。

デプロイは `main` ブランチのみで行い、`workflow_dispatch` による明示的な操作で起動する
（push では動かさない。誤って公開しないためである）。

**★公開の直前に検証をやり直す。** merge コミットの SHA は PR の SHA と異なり、
merge した結果そのものはまだ一度も検証されていない。`deploy.yml` の `precheck` が
同一コミットの検証結果を探し、24時間以内のものが無ければ検証を実行する。

`precheck` が検証を省くのは次の場合である。

| 場合                          | 理由                     |
| --------------------------- | ---------------------- |
| 同一SHAで CI が24時間以内に成功している    | fast-forward merge など  |
| 同一SHAの過去の公開で検証が通っている        | 失敗した公開のやり直し、フロントのみの再公開 |

**★実行全体ではなくジョブの結果を見る。** バックエンドで失敗した公開でも、
検証そのものは通っている。

## 11.2.1 ブランチ保護

`main` は保護する。設定内容は次のとおりである。

| 項目            | 値                          | 理由                                      |
| ------------- | -------------------------- | --------------------------------------- |
| 必須チェック        | `secrets-guard` / `verify` | CI が通っていない変更を main へ入れない                |
| 最新化の強制（strict） | 無効                         | main が動くたび PR の再検証が走り、CI の実行回数が増える       |
| PR の必須        | 有効                         | 直接 push を塞ぐ                              |
| 必要な承認数        | 0                          | **単独運用のため。** 1以上にすると自分のPRを承認できず、恒久的に詰まる |
| 管理者にも適用       | **有効**                     | 下記                                      |
| force push    | 禁止                         | 履歴を書き換えない                               |
| ブランチ削除        | 禁止                         | 同上                                      |

**★「管理者にも適用」を有効にしなければ保護は成立しない。** 無効のままだと、
リポジトリの所有者による直接 push は警告こそ出るが**そのまま通る**。
push する人が所有者だけの構成では、保護が事実上何もしていない状態になる。

### 緊急時

CI 自体が壊れて PR を通せない場合は、保護を一時的に外す。

```bash
# 「管理者にも適用」を外す（所有者の直接 push が通るようになる）
...
**★戻し忘れに注意する。** 外したままだと、以後の直接 push が黙って通る。
