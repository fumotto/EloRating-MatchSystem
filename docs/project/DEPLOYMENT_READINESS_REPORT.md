...
**実施日:** 2026-08-16
**ステータス:** ⚠️ 結論を訂正済み（下記「事後訂正」を先に読むこと）
...
**本報告書の結論「準備完了」「大きな不整合なし」は誤りであった。** 実地検証の結果、
本報告書自身が見落とした欠陥と、本報告書が新規作成した文書の欠陥が見つかった。

本文は当時の記録として残す。以下の訂正が本文に優先する。

## 訂正1：本報告書が作成した文書のSQLが動かなかった

「ドキュメントと実装の整合性検証」で `✅ 整合` と判定していたが、**新規作成した3文書の
SQLを実データベースへ流すと、ほぼ全件が失敗した。** 存在しない列・テーブルを参照していた。

| 記載 | 実際 |
| --- | --- |
| `matches.result_status` / `'pending'` | 列が無い。`status` で値は `PLAYING` / `WINNER_REPORTED` / `COMPLETED` / `DRAWN` |
| `matches.updated_at` | 列が無い |
| `matching_queue.status = 'waiting'` | 状態列が無い（行の存在が待機） |
| `matching_queue.created_at` | `queued_at` |
| `team_rating_history` | `rating_history` |
| `auth_logs` / `rating_queue` | 存在しない |
| `cron.job.last_successful_run` / `last_run_success` | 列が無い |
| `cron.job_run_details.error_message` | `return_message` |
| `cron.job_run_details.jobname` | 列が無い（`cron.job` と結合が必要） |

**原因は検証方法である。** 実装ファイルの存在と名称は照合していたが、**書いたSQLを実行して
いなかった。** 障害時に開く文書として成立していなかった。

訂正後は、掲載する全SQLを実データベースで実行して確認する方針とし、
`monitoring_guide.md` 1.1 に条件として明記した。

## 訂正2：本番の重大な欠陥を検出できていなかった

本報告書は Vault シークレット登録を「Staging デプロイ時」の**未来のタスク**として
挙げているが、実際には**既にデプロイ済みの本番環境で未登録のまま稼働しており**、
Cron が Edge Function を一度も呼んでいなかった（Issue #3）。

期限切れの試合が永久に確定せず、当事者は以後まったくマッチングできない状態であった。
これは R-004 が想定する最悪の結果である。

**検出できなかった理由：** 稼働中の環境に対する確認を行っていない。本報告書の検査は
リポジトリ内のファイルに閉じていた。

## 訂正3：実装にない画面操作を検証手順に書いていた

`deployment_checklist.md` のStaging検証項目に「スコア入力」「招待を送信」「成績」が
あったが、いずれも実装に存在しない。勝敗のみを記録する仕組みであり、招待はコードを
手渡す方式である。

また、マッチング検証には各チームが必須人数（既定3人）を満たす必要があり、
**最低6アカウントが要る**ことが書かれていなかった。

## 現在の状態

訂正1・3は修正済み（3文書のSQL 24本すべてを実データベースで実行確認）。
訂正2の Vault 登録は完了し、`net._http_response` に `200` を確認した。

Issue #3 の残作業として、**期限超過件数のアラート手段が未決**である。
現状は自動通知が無く、デプロイしない期間は無監視である。

## 教訓

**監査は実行して確かめる。** ファイルの存在と名称の照合では、動かないSQLも、
稼働環境の設定漏れも検出できない。
