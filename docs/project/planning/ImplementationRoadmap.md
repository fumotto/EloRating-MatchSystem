...
Cron ジョブが登録されていること・実行記録が `succeeded` であることは、
本条件を満たした証拠にならない。`invoke_edge_function` は Vault が未登録なら
何もせずに戻るため、**呼び出しが0回でもジョブは成功と記録される**（Issue #3）。

判定は「期限を過ぎたまま残る試合が増え続けないこと」で行う。
基準の詳細は `RiskManagement.md` 6.1.1、確認手段は `scripts/health-check.sql` である。

ローカルでは Vault 未登録が正常であるため、**本条件はクラウド（Staging）でのみ実証できる。**
S5 の実装は完了しているが、この一点は作業6（Vault 登録）の完了後に確認する。
