...
| 文書              | 責務                | 正本範囲                                    |
| --------------- | ----------------- | --------------------------------------- |
| ForkGuide.md    | 第三者がforkして自前で運用するための手順書 | **fork利用者向けの導入手順。非技術者を対象読者とする**  |
| PlayerGuide.md  | 選手（エンドユーザー）向けの使い方 | **画面の操作手順と業務ルールの説明。非技術者を対象読者とする**    |

`ForkGuide.md` は `SetupRunbook.md` と対象読者が異なる。前者はSEでない第三者、後者は本プロジェクトの開発者である。
手順が重複する箇所があるが、要求する前提知識が異なるため統合しない。値の定義は持たず、`11_Deployment.md` 4章を実務手順へ翻訳したものである。

`PlayerGuide.md` は仕様の正本ではない。期限・回数・レート計算などの値は `03_Database.md` 17章と
`08_RatingSpecification.md` が正本であり、本書はそれを利用者向けの言葉へ翻訳したものである。
**運営が `system_settings` を変更すれば実際の値は変わる**ため、本書は具体値を断定せず初期設定として示す。
